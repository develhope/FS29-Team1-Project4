...
export function CardProfessional() {
  ...
  return (
    <div className="custom-class">
      <Slider {...settings}>
        {db.map((card) => (
          <div key={card.id} className=" h-full text-black rounded-xl  m-2 my-shadow">
            <div className="h-36 rounded-t-xl flex justify-center items-center bg-[#004d99]">
            ...
            <div className="h-72 flex flex-col justify-between items-center text-center p-2">
              ...
              <button onClick={() => handleShowPopup(card)} className="bg-[#004d99] text-white text-lg p-2 rounded-lg">
                Scopri Di Più
              </button>
            </div>
          </div>
